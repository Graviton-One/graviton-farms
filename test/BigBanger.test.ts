...
describe("BigBanger", function () {
  ...
  it("should set correct state variables", async function () {
    this.chef = await this.BigBanger.deploy(this.relictGton.address, this.dev.address, "1000", "0", "1000")
    await this.chef.deployed()

    await this.relictGton.transferOwnership(this.chef.address)

    const relictGton = await this.chef.relictGton()
    const owner = await this.relictGton.owner()

    expect(relictGton).to.equal(this.relictGton.address)
  ...
  context("With ERC/LP token added to the field", function () {
    ...
    it("should allow emergency withdraw", async function () {
      ...
      this.chef = await this.BigBanger.deploy(this.relictGton.address, "100", "100", "1000")
      await this.chef.deployed()
    ...
    it("should give out SUSHIs only after farming time", async function () {
      ...
      expect(await this.relictGton.balanceOf(this.bob.address)).to.equal("0")
      ...
      expect(await this.relictGton.balanceOf(this.bob.address)).to.equal("1000")

      await advanceBlockTo("104")
      await this.chef.connect(this.bob).deposit(0, "0") // block 105

      expect(await this.relictGton.balanceOf(this.bob.address)).to.equal("5000")
      expect(await this.relictGton.totalSupply()).to.equal("5500")
    })

    it("should not distribute SUSHIs if no one deposit", async function () {
      // 100 per block farming rate starting at block 200 with bonus until block 1000
      this.chef = await this.BigBanger.deploy(this.sushi.address, "100", "200", "1000")
      await this.chef.deployed()
      await this.relictGton.transferOwnership(this.chef.address)
      await this.chef.add("100", this.lp.address, true)
      await this.lp.connect(this.bob).approve(this.chef.address, "1000")
      await advanceBlockTo("199")
      expect(await this.relictGton.totalSupply()).to.equal("0")
      ...
      expect(await this.relictGton.totalSupply()).to.equal("0")
      expect(await this.relictGton.balanceOf(this.bob.address)).to.equal("0")
      expect(await this.relictGton.balanceOf(this.dev.address)).to.equal("0")
      ...
      expect(await this.relictGton.totalSupply()).to.equal("11000")
      expect(await this.relictGton.balanceOf(this.bob.address)).to.equal("10000")
      expect(await this.relictGton.balanceOf(this.dev.address)).to.equal("1000")
      expect(await this.lp.balanceOf(this.bob.address)).to.equal("1000")
    })

    it("should distribute SUSHIs properly for each staker", async function () {
      // 100 per block farming rate starting at block 300 with bonus until block 1000
      this.chef = await this.BigBanger.deploy(this.sushi.address, "100", "300", "1000")
      await this.chef.deployed()
      await this.relictGton.transferOwnership(this.chef.address)
      await this.chef.add("100", this.lp.address, true)
      ...
      await this.chef.connect(this.alice).deposit(0, "10", { from: this.alice.address })
      expect(await this.relictGton.totalSupply()).to.equal("11000")
      expect(await this.relictGton.balanceOf(this.alice.address)).to.equal("5666")
      expect(await this.relictGton.balanceOf(this.bob.address)).to.equal("0")
      expect(await this.relictGton.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.relictGton.balanceOf(this.chef.address)).to.equal("4334")
      expect(await this.relictGton.balanceOf(this.dev.address)).to.equal("1000")
      // Bob withdraws 5 LPs at block 330. At this point:
      //   Bob should have: 4*2/3*1000 + 2*2/6*1000 + 10*2/7*1000 = 6190
      await advanceBlockTo("329")
      await this.chef.connect(this.bob).withdraw(0, "5", { from: this.bob.address })
      expect(await this.relictGton.totalSupply()).to.equal("22000")
      expect(await this.relictGton.balanceOf(this.alice.address)).to.equal("5666")
      expect(await this.relictGton.balanceOf(this.bob.address)).to.equal("6190")
      expect(await this.relictGton.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.relictGton.balanceOf(this.chef.address)).to.equal("8144")
      expect(await this.relictGton.balanceOf(this.dev.address)).to.equal("2000")
      ...
      expect(await this.relictGton.totalSupply()).to.equal("55000")
      expect(await this.relictGton.balanceOf(this.dev.address)).to.equal("5000")
      // Alice should have: 5666 + 10*2/7*1000 + 10*2/6.5*1000 = 11600
      expect(await this.relictGton.balanceOf(this.alice.address)).to.equal("11600")
      // Bob should have: 6190 + 10*1.5/6.5 * 1000 + 10*1.5/4.5*1000 = 11831
      expect(await this.relictGton.balanceOf(this.bob.address)).to.equal("11831")
    ...
    it("should give proper SUSHIs allocation to each pool", async function () {
      ...
      this.chef = await this.BigBanger.deploy(this.sushi.address, "100", "400", "1000")
    ...
    it("should stop giving bonus SUSHIs after the bonus period ends", async function () {
      ...
      this.chef = await this.BigBanger.deploy(this.sushi.address, "100", "500", "600")
      await this.sushi.transferOwnership(this.chef.address)
      await this.lp.connect(this.alice).approve(this.chef.address, "1000", { from: this.alice.address })
      ...
      expect(await this.relictGton.balanceOf(this.alice.address)).to.equal("10600")
    })
  })
})
